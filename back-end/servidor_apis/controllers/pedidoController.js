...
class PedidoController {
  ...
  async listar(req, res){
    const resultado = await pedidoModel.find({'userId': req.params.userId});
    res.status(200).json(resultado);
  }

  async buscarPorCodigo(req, res){
    const { userId, codigo } = req.params;
    const resultado = await pedidoModel.findOne({'codigo': codigo, 'userId' : userId});
    res.status(200).json(resultado);
  }
